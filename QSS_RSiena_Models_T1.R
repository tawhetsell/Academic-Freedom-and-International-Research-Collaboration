...
# ---- TABLE 2 SAOM, ST, SS, AH comparison ----
...
# remove SML and HKG from acadvar
...
# read democracy data for correlation test between acad and dem

country_names <-rownames(acadvar) 

demvar <- vdemdata::vdem %>%
  select(country_text_id, year, v2x_libdem) %>%
  filter(year >= 1993 & year <= 2022) %>%
  pivot_wider(names_from = year, values_from = v2x_libdem) %>%
  filter(country_text_id %in% country_names)

all_countries <- tibble(country_text_id = country_names)
result <- all_countries %>%
  left_join(demvar, by = "country_text_id")

demvar_wide <- result %>%
  column_to_rownames("country_text_id")
demvar_wide <- as.matrix(demvar_wide)

dem <- varCovar(demvar_wide)

# correlate dem and acad

a_d_data <- vdemdata::vdem %>%
  filter(country_text_id %in% country_names, year >= 1993 & year <= 2022) %>%
  select(country_text_id, year, v2x_libdem, v2xca_academ) %>% # Select specified variables
  arrange(country_text_id, year)
correlation_result <- cor(a_d_data$v2xca_academ, a_d_data$v2x_libdem, use = "complete.obs")
print(correlation_result)

# mixed effect 

model <- lmer(v2xca_academ ~ v2x_libdem + (1 | country_text_id), data = a_d_data, REML = FALSE)
summary(model)

# fixed effect

pdata <- pdata.frame(a_d_data, index = c("country_text_id", "year"))
fe_model <- plm(v2xca_academ ~ v2x_libdem, data = pdata, model = "within", effect = "twoways")
summary(fe_model)

# create degree centrality based on disparity matrix transformed matrixes

years_deg <- 1993:2022  
country_names_deg <- rownames(binary_matrices_list_ST$`1993`) 
deg_cent_df <- data.frame(matrix(ncol = length(years_deg), nrow = length(country_names_deg)))
rownames(deg_cent_df) <- country_names_deg
colnames(deg_cent_df) <- as.character(years_deg)

for (i in seq_along(years_deg)) {
  year_deg <- years_deg[i]
  net <- network(binary_matrices_list_ST[[as.character(year_deg)]], directed = FALSE)
  degree_centrality <- degree(net)
  deg_cent_df[, i] <- degree_centrality
}

# view(deg_cent_df)

# for correlation requested by reviewer, Pubs and Auth are 99 percent correlated

dist_df <- as.data.frame(log((distmatrix1/1000) + 1))
free_df <- as.data.frame(as.matrix(acadvar))
gdp_df <- as.data.frame(log(as.matrix(gdp_data_ppp_wide + 1)))
pop_df <- as.data.frame(log(as.matrix(population_data_wide + 1)))
urb_df <- as.data.frame(as.matrix(urbanization_data_wide))
deg_df <- as.data.frame(as.matrix(log(deg_cent_df + 1)))

dist_avg <- rowMeans({diag(dist_df) <- NA; dist_df}, na.rm = TRUE)
free_avg <- rowMeans(free_df, na.rm = TRUE)
gdp_avg <- rowMeans(gdp_df, na.rm = TRUE)
pop_avg <- rowMeans(pop_df, na.rm = TRUE)
urb_avg <- rowMeans(urb_df, na.rm = TRUE)
deg_avg <- rowMeans(deg_df, na.rm = TRUE)
pubs_avg <- rowMeans(log(WOS_pubs+1), na.rm = TRUE)
lang_avg  <- rowMeans({diag(lang_matrix) <- NA; lang_matrix}, na.rm = TRUE)
glob_avg <- rowMeans(glob_matrix, na.rm = TRUE)
...
correlation_matrix <- cor(combined_df1, use = "complete.obs")
cor_matrix_long <- reshape2::melt(correlation_matrix)
...
ggplot(cor_matrix_long, aes(Var1, Var2, fill = value)) +
  ...
  labs(title = "Correlation Matrix", x = NULL, y = NULL, fill = "Coefficient")

# Run regression models to show vifs 

model_a <- lm(deg_avg ~ dist + glob + lang + free + gdp + pop + urb + pubs, data = combined_df)
summary(model_a)
vif(model_a)

model_b <- lm(deg_avg ~ dist + glob + lang + free + gdp + pop + urb , data = combined_df)
summary(model_b)
vif(model_b)

# Generate some network statistics 

calculate_network_metrics_network <- function(binary_matrix) {
  net <- network(binary_matrix, directed = FALSE)  
  num_nodes <- network.size(net)
  num_edges <- network.edgecount(net)
...
network_metrics_over_time <- lapply(binary_matrices_list_ST, calculate_network_metrics_network)

network_df1 <- do.call(rbind, lapply(names(network_metrics_over_time), function(year) {
  metrics <- network_metrics_over_time[[year]]
  data.frame(
    year = year,
    nodes = metrics$number_of_nodes,
    edges = metrics$number_of_edges,
...
Model_T1_M1 <- siena07(IRC_algo, data=IRC_ST , effects = effects_ST, useCluster = TRUE, nbrNodes = 8)
Model_T1_M1

save(Model_T1_M1, file = "Model_T1_M1.Rdata")

# run model GOF diagnostics

simcontrols_ST <- sienaAlgorithmCreate(n3 = 400, nsub = 0, seed = 101, modelType = c(IRCnets_ST=3))
thesims_ST <- siena07(simcontrols_ST, 
                      data = IRC_ST, 
                      effects = effects_ST,
                      returnDeps = TRUE, 
                      prevAns = Model_T1_M1,
...
gofIndegrees_ST <- sienaGOF(thesims_ST, 
                            varName = "IRCnets_ST", 
                            IndegreeDistribution, 
                            cumulative = FALSE, 
                            levls = 0:15) 
plot(gofIndegrees_ST)
gofTriads_ST <- sienaGOF(thesims_ST, 
                         varName = "IRCnets_ST", 
                         TriadCensus, 
                         verbose = TRUE)
plot(gofTriads_ST, center = TRUE, scale = TRUE)
...
network_metrics_over_time <- lapply(binary_matrices_list_SS, calculate_network_metrics_network)
...
IRC_SS  <- sienaDataCreate(IRCnets_SS, dist, lang, free, glob, gdp, pop, urb)
...
Model_T1_M2 <- siena07(IRC_algo, data=IRC_SS , effects = effects_SS, useCluster = TRUE, nbrNodes = 8)
Model_T1_M2

save(Model_T1_M2, file = "Model_T1_M2.Rdata")

# run GOF

simcontrols_SS <- sienaAlgorithmCreate(n3 = 400, nsub = 0, seed = 101, modelType = c(IRCnets_SS=3))
thesims_SS <- siena07(simcontrols_SS, 
                      data = IRC_SS, 
                      effects = effects_SS,
                      returnDeps = TRUE, 
                      prevAns = Model_T1_M2,
                      useCluster = TRUE,  
                      nbrNodes = 8)         
gofIndegrees_SS <- sienaGOF(thesims_SS, 
                            varName = "IRCnets_SS", 
                            IndegreeDistribution, 
                            cumulative = FALSE, 
                            levls = 0:15) 
plot(gofIndegrees_SS)
gofTriads_SS <- sienaGOF(thesims_SS, 
                         varName = "IRCnets_SS", 
                         TriadCensus, 
                         verbose = TRUE)
plot(gofTriads_SS, center = TRUE, scale = TRUE)
...
network_metrics_over_time <- lapply(binary_matrices_list_AH, calculate_network_metrics_network)
...
IRC_AH  <- sienaDataCreate(IRCnets_AH, dist, lang, free, glob, gdp, pop, urb)
...
Model_T1_M3 <- siena07(IRC_algo, data=IRC_AH , effects = effects_AH, useCluster = TRUE, nbrNodes = 8)
Model_T1_M3

save(Model_T1_M3, file = "Model_T1_M3.Rdata")

# if convergence bad run with prevans

# IRC_algo <- sienaAlgorithmCreate(n3=9000, seed=101, modelType = c(IRCnets_AH=3))
# Model_T1_M3_p <- siena07(IRC_algo, data=IRC_AH , effects = effects_AH, prevAns = Model_T1_M3,  useCluster = TRUE, nbrNodes = 8)
# Model_T1_M3_p

# run GOF

simcontrols_AH <- sienaAlgorithmCreate(n3 = 400, nsub = 0, seed = 101, modelType = c(IRCnets_AH=3))
thesims_AH <- siena07(simcontrols_AH, 
                      data = IRC_AH, 
                      effects = effects_AH,
                      returnDeps = TRUE, 
                      prevAns = Model_T1_M3,
                      useCluster = TRUE,  
                      nbrNodes = 8)         
gofIndegrees_AH <- sienaGOF(thesims_AH, 
                            varName = "IRCnets_AH", 
                            IndegreeDistribution, 
                            cumulative = FALSE, 
                            levls = 0:15) 
plot(gofIndegrees_AH)
gofTriads_AH <- sienaGOF(thesims_AH, 
                         varName = "IRCnets_AH", 
                         TriadCensus, 
                         verbose = TRUE)
plot(gofTriads_AH, center = TRUE, scale = TRUE)
...
# load RData files for T1
load("Model_T1_M1.Rdata")
load("Model_T1_M2.Rdata")
load("Model_T1_M3.Rdata")

# Create model group for T1
model_group_T1 <- list(Model_T1_M1, Model_T1_M2, Model_T1_M3)
...
effect_names <- Model_T1_M1$effects$effectName

# make p-values and categories for significance (p_cat)
...
generate_latex_table <- function(model_group, model_names, effect_names, custom_names, p_cat) {
  ...
  # add the table notes row, before the end of the table
...
# generate the LaTeX table for model group T1
latex_table_T1 <- generate_latex_table(model_group_T1, model_names_T1, effect_names, custom_variable_names, p_cat_T1)

# save the LaTeX table to a file
cat(latex_table_T1, file = "latex_table_T1.tex")
